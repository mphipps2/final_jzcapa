...
G4bool FastFiberModel::ModelTrigger(const G4FastTrack& fasttrack) {
  ...
  //  std::cout <<" fiberEnd:: " << fiberEnd << " toEnd " << toEnd << " mNtransport " << mNtransport << std::endl;
  ...
  if ( checkAbsorption(mDataPrevious.GetAbsorptionNILL(), mDataCurrent.GetAbsorptionNILL()) ) {
    ...
    std::cout << "absorbed before fiber end " << std::endl;
  }
...
void FastFiberModel::DoIt(const G4FastTrack& fasttrack, G4FastStep& faststep) {
  ...
  if (fKill) { // absorption
    ...
    std::cout << " killing primary track " << std::endl;
    return;
  }

  if (fTransported) {
    std::cout <<" NILL check " << std::endl;
...
G4bool FastFiberModel::checkTotalInternalReflection(const G4Track* track) {
  ...
  if (fVerbose>1) {
    ...
    G4cout << " | G4OpBoundaryProcessStatus = " << std::setw(2) << theStatus << " TIR: " << G4OpBoundaryProcessStatus::TotalInternalReflection  << " too small step: " << G4OpBoundaryProcessStatus::StepTooSmall;
  ...
  if ( theStatus==G4OpBoundaryProcessStatus::TotalInternalReflection || theStatus==G4OpBoundaryProcessStatus::StepTooSmall ) {
    ...
    G4double incidentAngle = pOpBoundaryProc->GetIncidentAngle(); 
    G4double incidentDegrees = (incidentAngle / (2*M_PI)) * 360;
    std::cout << " incident angle from FastFiberModel!   " << incidentDegrees << " G4OpBoundaryProcessStatus = " << std::setw(2) << theStatus << " logicalVol " << track->GetVolume()->GetLogicalVolume()->GetName() << " Position " << track->GetPosition() << " stepNum " << track->GetCurrentStepNumber() << " StepLength = " << std::setw(9) << track->GetStepLength() << std::endl;
