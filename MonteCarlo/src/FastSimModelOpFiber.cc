...
G4bool FastSimModelOpFiber::ModelTrigger(const G4FastTrack& fasttrack) {
  ...
  if ( fTransportUnit < 0. ) { // kill backward propagation
    std::cout << " fTransportUnit " << fTransportUnit << " theta " << theta << std::endl;
    fKill = true;
  ...
  //  std::cout << " fNTransport " << fNtransport << " maxTransport " << maxTransport << " fSafety " << fSafety << " toEndAxis " << toEndAxis << " fTransportUnit " << fTransportUnit << " theta " << theta << std::endl;
...
bool FastSimModelOpFiber::checkTotalInternalReflection(const G4Track* track) {
  ...
  //  std::cout << " incident angle from FastFiberModel!   " << incidentDegrees << " G4OpBoundaryProcessStatus = " << std::setw(2) << theStatus << " logicalVol " << track->GetVolume()->GetLogicalVolume()->GetName() << " Position " << track->GetPosition() << " stepNum " << track->GetCurrentStepNumber() << " StepLength = " << std::setw(9) << track->GetStepLength() << " TrackID " << track->GetTrackID() << std::endl;
