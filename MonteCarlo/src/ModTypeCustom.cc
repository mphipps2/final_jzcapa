...
ModTypeCustom::ModTypeCustom(const int cn,const G4ThreeVector& pos,
...
void ModTypeCustom::DefineMaterials()
{
  // Get Config
...
void ModTypeCustom :: DefineBorderProperties(){
  const G4int NUMENTRIES = 2;
  G4double ephoton[NUMENTRIES] = {2.00*eV,4.80*eV};
  // Get Config
  //  TEnv* config = m_sd->GetConfig();
...
void ModTypeCustom::ConstructDetector()
{
  // Get Config
  TEnv* config = m_sd->GetConfig();
  ...
  for (int i = 0; i < nModules; ++i) {
    ...
    modType[i] = config->GetValue(variable,4);    
    if (modType[i] == 4) {
      ...
      sprintf(variable,"mod%dAbsorberMat",i+1);
  ...
  m_StripTube                     = new G4Tubs( "Strip_Tube", 0.0*mm     , ( modCoreDiameter[m_modNum]/2.0-0.005)*mm  , modHeightY*mm/2.0 , 0.0*deg, 360.0*deg);
  m_StripLogical                         = new G4LogicalVolume(m_StripTube          ,m_matQuartz, "Strip_Logical");
  if (cladding[m_modNum]) {
    m_CladdingTube                     = new G4Tubs( "Cladding_Tube", modCoreDiameter[m_modNum]/2.0*mm - (0.005)*mm     , (modCoreDiameter[m_modNum]/2.0*mm-0.0005*mm+modCladdingThickness[m_modNum])*mm  , modHeightY*mm/2.0, 0.0*deg, 360.0*deg);
  ...
  for(int K=0;K<modNRadiators[m_modNum];K++) {  
    for(int M=0;M<modNStripsPerGap[m_modNum];M++) { 
      //      cn = M + K*modNStripsPerGap[m_modNum] + m_modNum*modOffset;      
      sprintf(name,"Strip_a %d", cn);
      m_StripPhysical[K][M] = new G4PVPlacement(stripRotation,G4ThreeVector((xStartStrip+(M*stripPitch))*mm,0,(zStartRad+K*zPitch)*mm),m_StripLogical,name,m_ModuleLogical,false,cn,checkOverlaps);     
      if (cladding[m_modNum]) {
	sprintf(name,"Cladding_a %d", cn);
	m_CladdingPhysical[K][M] = new G4PVPlacement(stripRotation,G4ThreeVector((xStartStrip+(M*stripPitch))*mm,0,(zStartRad+K*zPitch)*mm),m_CladdingLogical,name,m_ModuleLogical,false,cn,checkOverlaps);
      }
      ++cn;
    }
  }
  ...
  */
  std::cout << "ModTypeCustom construction finished: SD name " << quartzSDname << std::endl;
